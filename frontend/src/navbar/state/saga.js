...
import { callApiMovieList, callApiGanreMovieList, callApiCountryMovieList, callApiContentMovieList } from "../../common/api";
...
export function* countryData(action) {
  ...
  yield put(actions.setLoading(true));
  try {
    const data = yield call(callApiCountryMovieList, action.ganre, action.pageNum);
    if (data !== undefined) {
      yield put(actions.setMovieList(data));
    }
  } catch(error) {
